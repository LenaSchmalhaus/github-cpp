...
void ostrcpy(char *ziel, const char *quelle) {
    int z = 0;
    do {
        ziel[z] = quelle[z];                        // hier ist z 0
        z++;                                        // hier ist z schon 1
    } while (quelle[z - 1] != '\0');                // deswegen zieht man 1 ab, da die Schleife einen Schritt zu weit zählt!!!
}

//2
int ostrlen(char *a) {
    int i = 0;
    while (a[i++]);                                // die while Schleife increment schon in der Bedingung und benötigt dann keine geschweiften Klammern
    return i;                                      //  ohne Endekennung wäre i-1;
}
...
void midstr9(char*ziel, char*quelle, int pos, int anz)
{
    int laengeq = ostrlen(quelle);           // Länge des Quellstrings bestimmen
    int laengez = ostrlen(ziel);             // Länge des Zielstrings bestimmen



    if (anz > (laengeq -pos))               // Sicherstellen, dass nicht mehr Zeichen kopiert werden als vorhanden
    { anz = laengeq - pos;}                 // Reduziere anz, um nicht über den Quellstring hinauszukopieren

            for (int i = 0; i < anz; i++)         // Zeichen ab der Position `pos` im Quellstring in den Zielstring ab der Position `laengez` kopieren
            {
                ziel[i] = quelle[pos + i];        // Kopiere Zeichen ans Ende des Zielstrings
            }

    ziel[laengez + anz] = '\0';                   // Endekennung setzen, um den Zielstring korrekt zu beenden
    cout << ziel << endl;                   // Zielstring ausgeben
}
...
// void eingabe(int*feld, int laenge)
//
